...
## im new to github and such so if i do smth wrongly with commoits and such please let me know
## This script is a draft. The goal is to deploy Kazoo v3.0

## needs to be altered or removed as we will ask the user for the data
...
echo "welcome to this kazoo single server installer"
echo "your current hostname is" 
hostname
echo " i dont know how to skip stuff if this is correct, but we can set the hostname so i will go on"

echo "note to self: learn bashmore, to ask if is corect or not and then take actions....."
#that would be a nice function

echo " this script asumes that the hostname is the same as the fqdn"

clear echo "Please Enter your Desired Hostname" 
read HOSTNAME 
...
#lets set the account name, realm, user, password
echo "please specify the accountname, realm (fqdn/hostname?) user and pass"
echo "lts start with the acountname (we suggest a simple name, u need it to login later)"
read acount_name
echo "enter the realm (hostname/fqdn)"
read realm
echo " enter the username"
read user
echo "enter the users paword"
read pass

echo "thanks, that should do it"
echo "please hang on while i do my best"
