...
decode(JSON, <<"application/json">>) ->
    try ejson:decode(JSON) of
        ...
        'throw':{'error',{_Loc, 'invalid_string'}}=E ->
            io:format("invalid string(near char ~p) in input, checking for unicode~n", [_Loc]),
            try_converting(JSON, E);
        'throw':{'invalid_json',{{'error',{1, Err}}, _Text}} ->
            io:format("~s: ~s~n", [Err, _Text]),
            throw({'error', Err});
        'throw':{'invalid_json',{{'error',{Loc, Err}}, Text}} ->
            Size = erlang:byte_size(Text),
            Part = binary:part(Text, Loc, Size-Loc),
            io:format("~s near char # ~b of ~b ('~s'): ~s~n", [Err, Loc, Size, Part, Text]),
            try_converting(JSON, {'error', Err});
        'throw':E ->
            io:format("thrown decoder error: ~p~n", [E]),
            throw(E)
    end.
try_converting(JSON, E) ->
    case unicode:bom_to_encoding(JSON) of
        {'latin1', 0} ->
            io:format("json is latin1, trying as utf8~n"),
            case unicode:characters_to_binary(JSON, 'latin1', 'utf8') of
                ...
                _O -> io:format("failed to char_to_bin: ~p~n", [_O]), throw(E)
            end;
        _Enc ->
            io:format("unknown encoding: ~p~n", [_Enc]),
            throw(E)
    end.
...
encode_kv(Prefix, K, Vs) when is_list(Vs) ->
    encode_kv(Prefix, wh_types:to_binary(K), Vs, <<"[]=">>, []);
...
encode_kv(<<>>, K, Sep, V) -> [wh_types:to_binary(K), Sep, wh_types:to_binary(V)];
encode_kv(Prefix, K, Sep, V) -> [Prefix, <<"[">>, wh_types:to_binary(K), <<"]">>, Sep, wh_types:to_binary(V)].
...
get_binary_value(Key, JObj, Default) ->
    case get_value(Key, JObj) of
        'undefined' -> Default;
        Value -> wh_types:to_binary(Value)
    end.
...
get_integer_value(Key, JObj) ->
    case get_value(Key, JObj) of
        'undefined' -> 'undefined';
        Value -> wh_types:to_integer(Value)
    end.
...
get_number_value(Key, JObj) ->
    case get_value(Key, JObj) of
        'undefined' -> 'undefined';
        Value -> wh_types:to_number(Value)
    end.
...
is_false(Key, JObj) ->
    wh_types:is_false(get_value(Key, JObj)).

is_false(Key, JObj, Default) ->
    case get_value(Key, JObj) of
        'undefined' -> Default;
        V -> wh_types:is_false(V)
    end.
...
get_binary_boolean(Key, JObj, Default) ->
    case get_value(Key, JObj) of
        'undefined' -> Default;
        Value -> wh_types:to_binary(wh_types:is_true(Value))
    end.
...
get_ne_value(Key, JObj, Default) ->
    ...
    case wh_types:is_empty(Value) of
        'true' -> Default;
        'false' -> Value
    end.
...
get_value([Key|Ks], L, Default) when is_list(L) ->
    try
        get_value1(Ks, lists:nth(wh_types:to_integer(Key), L), Default)
    catch
...
get_value1([K|Ks], JObjs, Default) when is_list(JObjs) ->
    try lists:nth(wh_types:to_integer(K), JObjs) of
        'undefined' -> Default;
...
prune(_, []) -> [];
prune([K|T], [_|_]=JObjs) ->
    V = lists:nth(wh_types:to_integer(K), JObjs),
    case prune(T, V) of
...
no_prune([K|T], Array) when is_list(Array) ->
    {Less, [V|More]} = lists:split(wh_types:to_integer(K)-1, Array),
...
load_fixture_from_file(App, File) ->
    Path = list_to_binary([code:priv_dir(App), "/couchdb/", wh_types:to_list(File)]),
    io:format("read fixture from filesystem whapp ~s from CouchDB JSON file: ~s~n", [App, Path]),
    try
        {'ok', Bin} = file:read_file(Path),
        decode(Bin)
    catch
        _Type:{'badmatch',{'error',Reason}} ->
            io:format("badmatch error: ~p~n", [Reason]),
            {'error', Reason};
        _Type:Reason ->
...
to_querystring_test() ->
    ...
    lists:foreach(fun({JSON, QS}) ->
                          QS1 = wh_types:to_binary(to_querystring(decode(JSON))),
                          ?assertEqual(QS, QS1)
                  end, Tests).
